...
const questions = listQuestions.map(el => {
	...
	result.caption = result.title.match(/^\d\d (.+)/)[1].trim();
	// Find the question text by moving past the blank lines
	var elQuestion = el.nextElementSibling, textQuestion;
	while(textQuestion = elQuestion.textContent.replace(/\s+/mg," ").trim(),textQuestion === "" || textQuestion.substr(0,10) === "Valoración" || textQuestion.substr(0,8) === "Problema") {
